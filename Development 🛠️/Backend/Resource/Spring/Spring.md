...
### 프로덕션 준비
 ...
 - 설명
	 - 제품을 고객에게 제공하기 위해 준비하는 애플리케이션 외적의 비기능적 요구사항 활동
	 - 액추에이터, 프로메테우스, 그라파나, 마이크로미터등의 라이브러리를 활용해 로깅, 감사, 모니터링등의 활동을 할 수 있다.
...
- 설명
	- 스프링이 제공하는 프로덕션 준비 기능을 모은 라이브러리
	- JMX나 HTTP를 통해 각 기능 별 엔드포인트를 제공하여 애플리케이션 기능 관리 및 모니터링을 할 수 있다.

- 자주 사용되는 주요 엔드 포인트
	- beans
		- 스프링 컨테이너에 등록된 빈을 보여준다.
	- conditions
		- condition 어노테이션을 통해 빈 등록시 평가 조건과 일치하거나 일치하지 않는 이유를 보여준다.
	- configprops
		- ConfigurationProperties 어노테이션을 보여준다.
	- env
		- 환경 설정 정보를 보여준다.
	- health
		- 애플리케이션 활성 상태를 보여준다.
	- httpexchanges
		- HTTP 호출 응답 정보를 보여준다. HTTPExchangeRepository를 구현한 빈을 별도 등록을 해주어야한다.
	- info
		- 애플리케이션 정보를 보여준다.
	- loggers
		- 애플리케이션 로거 설정을 보여주고 변경도 가능하다.
	- metrics
		- 애플리케이션 메트릭 정보를 보여준다.
	- mappings
		- 열려있는 요청 매핑 정보를 보여준다.
...
- logger
	- 설명
		- 로그 레벨이 적용된 상태를 확인할 수 있음
		- json에 configuredLevel: "레벨"을 넣으면 레벨에 맞는 로그 레벨로 설정을 변환할 수 있다.
		- 현업에서는 로그를 정말 자주 찍어서 보는데, 일반적으로 디스크에 영향을 주기 때문에 info를 기본으로 두는 경우가 대다수이지만, 운영상 문제가 발생했을 때 log.debug를 확인해야하는 상황이 자주 발생한다. 이때 로그레벨을 바꾸기 위해 서버를 내렸다가 다시 올리지 않고도 로그 레벨을 변경할 수 있는 것이다.
...
- 설명
	- 모니터링 툴에 상관없이 측정 지표를 일관적으로 모니터링 툴에 제공하기 위해 사용하는 라이브러리
	- 이미 모니터링 툴 별로 구현체가 존재하여 이걸 그냥 가져다 사용하면 된다.
	- 이미 스프링부트 액추에이터에 마이크로미터를 가져다 사용하고 있다.

&emsp;&emsp;

- 메트릭 검색 방법
	- 태그를 이용하여 필터링을 거쳐서 조회하는 것이 가장 일반적이고 이것이 모니터링 툴에서 화면을 구성하는데 가장 유용한 방법이다.
	- 태그 검색 방법
		- `메트릭 주소?tag=키:값`
	- 주의점
		- 태그는 카디널리티가 낮은 단위에 사용해야한다.
			- 성별, 주문 상태, 결제 수단 등
			- 들어갈 수 있는 데이터의 가지 수가 정해져있고 얼마 없는 경우에 태그로 써야한다는 것이다.
...
- 제공 가능한 자주 쓰는 메트릭
	...
	- 스프링 MVC
	- 톰캣
		- 반드시 `server.tomcat.mbeanregistry.enabled: true`로 두는 것이 좋다.
			- 유용한 정보는 여기에 있는 편이다.
			- 기본적으로 세션에 대한 정보만 제공해준다.
		- 유용한 톰켓 매트릭
			- tomcat.threads.busy
				- 요청을 처리하고 있는 스레드를 보여준다.
			- tomcat.threads.config.max
				- 얼마나 많은 요청을 처리할 수 있는 스레드가 있는지 보여준다.
			- 위의 두 가지 매트릭이 항상 주시하고 있다가 점점 busy 스레드가 많아지다가 max에 도달하게 되면 그때부터 장애가 터지므로 이 busy 스레드와 max 스레드의 수의 비율을 주시하고 있는 것이 운영의 핵심이다.
	- 데이터 소스
	- 로그
	- 사용자 정의
		- 실제 운영에서는 핵심 비즈니스와 관련된 매트릭을 여기서 캐치하여 보여주는 것이 일반적이다.

&emsp;&emsp;

- 카운터 등록 방법
	1. `@Counter("카운터 이름")` 어노테이션을 넣고 싶은 서비스 메서드 레벨에 붙여준다.
	2. `CounterAspect` 빈을 등록한다.

&emsp;&emsp;

- 타이머 등록 방법
	1. `@Timed("타이머 이름")` 어노테이션을 넣고 싶은 서비스의 클래스 또는 메서드 레벨에 붙여준다.
	2. `TimedAspect` 빈을 등록한다.

&emsp;&emsp;

- 게이지 등록 방법
	1. Guage를 등록해야한다.
		1. 게이지는 현재의 특정 데이터의 상태 값을 확인하기 위함으로 존재하기 때문에 빌더에서는 외부에서 게이지를 확인하면 등록된 함수가 호출되도록 함수를 람다로 등록한다.
		2. 상품 재고량, 로그인된 유저 수 등이 대표적이다.
		3. `MeterBinder`를 빈으로 등록한다.
			1. `registry -> Gauge.builder("게이지 명", orderService, service ->{ return service.getUserCount()}).register(registry);` 이렇게 넣으면 된다.
			2. 서비스에서 확인하고 싶은 값을 넣으면 된다.
			3. 이 게이지를 자주 호출하진 말자. 커넥션 마른다.
...
- 설명
	- 메트릭 지표를 저장하고 제공하는 데이터 소스

- 프로메테우스 아키텍처
	- push 게이트웨이
		- 지속적으로 프로메테우스의 타겟(어플리케이션)들이 잡으로 등록되어 메트릭을 전달된다.
	- 프로메테우스 서버
		- 매트릭을 pull하여 TSDB에 저장한다.
	- 알람 매니저
		- 조건에 맞춰 프로메테우스 서버에서 알람을 전달받는다.
	- UI 툴 (그라파나 등)
		- 데이터 비주얼라이저가 PromQL을 이용해 데이터를 조회한다.
	- 서비스 탐색기
		- 프로메테우스 서버는 쿠버네티스 등을 통해 타겟을 지속적으로 찾는다.

- 앱 설정
	- 프로메테우스는 /actuator/metrics의 json 포멧은 이해하지 못하는데 이를 micrometer가 처리해주게 된다.
		- 프로메테우스는 actuator의 정보들 중 태그가 섞인 정보는 한 row 씩 풀어서 들고 온다.
...
- 매트릭 구성요소
	...
	- 카운터
		- 특징
			- 단순하게 증가하는 단일 누적 값
			- 앱 시작부터 누적된 정보
			- 특정 시간에 한눈에 얼마나 많은 고객이 들어왔는가를 보기에 상당히 힘들다.
				- 계속 올라가는 그래프이기 때문에 그렇다.
				- increase()나 rate() 함수를 사용해야 제대로 써먹을 수 있는 매트릭 데이터이다.
		- 예시
			- HTTP 요청 수, 로그 발생 수
		- 활용 예시
			- `increase(http_server_requests_seconds_count{uri="/log"}[1m])`
				- 백터 연산자를 동원하여 범위를 지정해야한다.
				- 이를 이용하면 어느 시간대에 가장 많이 고객이 모이는지, 요청했는지 등을 1분 단위로 볼 수 있다.
			- `rate()`
				- 벡터 연산자를 동원하여 범위를 지정해야한다.
				- 초당 평균 증가율을 계산하며 간단하게 increase를 초당 증가율로 변환한 버전이라고 보면 된다.
					- increase는 횟수를 표현했지만 rate는 비율로  y 좌표를 표현한다.
					- 정말 많은 수의 요청이 들락 거리는 서비스면 rate를 쓰는게 괜찮다.
			- `irate()`
				- 범위 벡터에서 초당 순간 증가율을 계산한다.
				- 보통 이벤트 요청처리와 같은 순간적으로 증가하는 처리량을 파악하는데 사용된다.
...
- 설정
	- Admin 설정
		...
		- 초기 로그인 비밀번호
			- admin
	- k8s 셋업 팁
		- deployment로 만들고 pvc를 엮어주면 된다.
			- 그냥 디폴트 동적 프로비저닝을 이용해주자
	- 프로메테우스 연동
		- 방법
			- k8s의 경우, 프로메테우스의 svc FQDN을 사용해주어야한다.
		- FQDN
			- `<서비스 명>.<서비스의 네임스페이스>.svc.cluster.local`
		- FQDN 검증
			- `kubectl run -i --rm --tty dns-test --image=busybox --restart=Never -- nslookup prometheus-ui.default.svc.cluster.local`
			- 여기서 룩업 결과가 안나온다면 FQDN이 잘못된 것이고 잘 나온다면 사용하면 된다.
...
1. 대시보드를 띄워 놓는다.
	1. 전체 시스템 상황을 늘 보고 있다가 문제가 발생하면 핀포인트로 앱을 확인한다.
2. 핀포인트와 같은 툴로 앱을 추적한다.
	1. 가장 에러를 빠르게 확인할 수 있는 방법은 핀포인트나 스카우트 같은 툴로 빠르게 해결이 가능하다.
	2. 콜스택을 열어서 바로 원인을 파악해볼 수 있다.
3. 로그를 확인한다.
	1. 로그는 사용자의 요청 구분을 위해 MDC 적용이 필수이다.
